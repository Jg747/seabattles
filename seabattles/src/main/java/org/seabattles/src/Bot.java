...
public class Bot extends Player {
	...
	public Bot(Map<Integer, ShipConfig> shipsConfig) {
		super(shipsConfig);
			...
			do {
				try {
					...
					if (Main.DEBUG_TESTING_DATA) {
						int[] ret = Main.getBotTestShipPlacement(s.getID());
						rndX = ret[0];
						rndY = ret[1];
						rndRot = ret[2];
					} else {
						rndRot = rndGen.nextInt(4);
						rndX = rndGen.nextInt(Board.getWidth());
						rndY = rndGen.nextInt(Board.getHeigth());
					}
