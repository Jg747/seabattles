...
public abstract class Protocol {
	...
	private static final Map<MsgType, String> types = Map.ofEntries(
			...
			Map.entry(MsgType.MATCH_HOST_OK, "match_host_ok"),
			...
			Map.entry(MsgType.SHIP_SUNK, "ship_sunk"),
	...
	private static Optional<JSONObject> putConfigInObj(JSONObject obj, GameConfig config) {
		...
		for (ShipConfig s : config.getShipsConfig().values()) {
		...
		cfg.put(strings.get(MsgString.DEBUG), config.getDebugString());
	...
	public static GameConfig parseConfigurationMessage(JSONObject msg) throws RuntimeException, ParseException {
		...
		Map<Integer, ShipConfig> ships = new TreeMap<>();
		for (int i = 0; i < shipsArr.length(); i++) {
			ShipConfig s = new ShipConfig(shipsArr.getJSONObject(i));
			if (!s.getSpritePath().equals(NULL)) {
				s.setSprite(Main.SERVER_SPRITE_PATH + "/" + i);
			}
			ships.put(s.getID(), s);
		}
		ret.setShipsConfigArray(ships);
		ret.setDebugString(cfg.getString(strings.get(MsgString.DEBUG)));
	...
	public static JSONObject getMatchHostOkMessage(boolean error) {
		JSONObject ret = new JSONObject();
		ret.put(strings.get(MsgString.TYPE), types.get(MsgType.MATCH_HOST_OK));
		ret.put(strings.get(MsgString.STATUS), error);
		return ret;
	}
	
	public static boolean parseMatchHostOkMessage(JSONObject msg) {
		return msg.getBoolean(strings.get(MsgString.STATUS));
	}
	...
	public static Optional<JSONObject> getConfigBoardMessage(Board board) {
		...
		JSONArray arr = new JSONArray();
		Map<Integer, Ship> ships = board.getShips();
	...
	public static Optional<Board> parseConfigBoardMessage(JSONObject msg, Map<Integer, ShipConfig> cfg) {
		Board ret = new Board(cfg);
		
		JSONArray arr = msg.getJSONArray(strings.get(MsgString.SHIPS));
		if (arr.length() != cfg.size()) {
			return Optional.empty();
		}
		
		Map<Integer, Ship> ships = ret.getShips();
		for (Object o : arr) {
			Ship s = ships.get(((JSONObject) o).getInt(strings.get(MsgString.ID)));
			s.rotate(((JSONObject) o).getInt(strings.get(MsgString.R)));
			s.setX(((JSONObject) o).getInt(strings.get(MsgString.X)));
			s.setY(((JSONObject) o).getInt(strings.get(MsgString.Y)));
		}
	...
	public static JSONObject getShipSunkMessage(UUID id, Ship ship) {
		JSONObject ret = new JSONObject();
		ret.put(strings.get(MsgString.TYPE), types.get(MsgType.SHIP_SUNK));
		ret.put(strings.get(MsgString.WHO), id.toString());
		ret.put(strings.get(MsgString.ID), ship.getID());
		ret.put(strings.get(MsgString.X), ship.getX());
		ret.put(strings.get(MsgString.Y), ship.getY());
		ret.put(strings.get(MsgString.R), ship.getRotation());
		return ret;
	}
	
	public static Object[] parseShipSunkMessage(JSONObject msg) {
		Object[] ret = new Object[5];
		
		ret[0] = UUID.fromString(msg.getString(strings.get(MsgString.WHO)));
		ret[1] = msg.getInt(strings.get(MsgString.ID));
		ret[2] = msg.getInt(strings.get(MsgString.X));
		ret[3] = msg.getInt(strings.get(MsgString.Y));
		ret[4] = msg.getInt(strings.get(MsgString.R));
